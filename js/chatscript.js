var baseurl = 'http://dailyneeds.co.nz';
socket = io.connect(baseurl+':3001', {'forceNew': true});
...
socket.on('getChatUsersListResponse', (data)=>{
	...
	if(data.data.length){
		...
		$('.ripple-effect').css('display', 'none');
		$('#usersData').html('<li>'
									+'<a href="#">'
										+'<div class="message-by">'
											+'<div class="message-by-headline">'
	...
	for (var i = 0; i < data.data.length; i++) {
		...
		var online = (data.data[i].status) ? 'status-online' : '';
	  	var users = 	'<li>'+
						...
						'				<span>'+data.data[i].time+'</span>'+
						'			</div>'+
						'			<p>'+data.data[i].messages[0].msg+'</p>'+
						'		</div>'+
						'	</a>'+
						'</li>';
		$('#usersData').append(users);
...
/*socket.on('getChatUsersResponse', (data)=>{
	console.log("data :::::: ", data);
	var output = [];
	for (var i = 0; i < data.data.length; i++) {
		if(i==0){
	...
	var AllMessages = "";
	userMessages = data.data[0].AllMessages;

	for (var j = 0; j < data.data[0].AllMessages.length; j++) {
		console.log(myid+" == "+data.data[0].AllMessages[j].senderid)
		if(myid == data.data[0].AllMessages[j].senderid){
			AllMessages = 	'<input type="text" id="opp" value="'+data.data[0].AllMessages[j].senderid+'" hidden>'+
						'<div class="message-bubble me">'+
						'	<div class="message-bubble-inner">'+
						'		<div class="message-avatar"><img src="'+data.data[0].AllMessages[j].profile+'" alt="" /></div>'+
						'		<div class="message-text"><p>'+data.data[0].AllMessages[j].msg+'</p></div>'+
						'	</div>'+
						'	<div class="clearfix"></div>'+
						'</div>';
			output.push(AllMessages);
	...
	$('.message-content-inner').html(output.join(''));
	// $(".simplebar-scroll-content simplebar-content").animate({ scrollBottom: $(document).height() }, 1000);
	// $(".message-content-inner").scrollTop(10000);
	$(".message-content-inner").animate({ scrollTop: $('.message-content-inner').prop("scrollHeight")}, 0);
})*/
